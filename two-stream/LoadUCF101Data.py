...
print(TrainVideoNameList)
TestVideoNameList = []
with open('testlist01.txt', 'r') as f:
    all_Class_and_Ind = f.readlines()
    for line in all_Class_and_Ind:
        
        videoname = line[:-1].split('.')[0]
        
        videoname = videoname.split('/')[1]
        TestVideoNameList.append(videoname)
...
class UCF101Data(Dataset):  # define a class named MNIST
    ...
    def __init__(self, RBG_root, OpticalFlow_root, isTrain, transform=None):
        ...
        for i in range(0, 4):
            OpticalFlow_class_path = '/home/hyq-user/xcy/Two-Stream-Network-PyTorch/data/OpticalFlow' + '/' + classInd[i]
            RGB_class_path = '/home/hyq-user/xcy/Two-Stream-Network-PyTorch/data/RGB' + '/' + classInd[i]
...
trainset = UCF101Data(
    RBG_root='/home/hyq-user/xcy/Two-Stream-Network-PyTorch/data/RGB',
    OpticalFlow_root='/home/hyq-user/xcy/Two-Stream-Network-PyTorch/data/OpticalFlow',
    isTrain=True,
    transform=transform
)
